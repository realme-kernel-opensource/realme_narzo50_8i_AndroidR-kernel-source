...
struct aw87319_t {
	...
	unsigned char dynamic_boost_sence;
	unsigned char spk_cfg_update_flag;
	unsigned char vspk_cfg_update_flag;
	unsigned char bspk_cfg_update_flag;
...
struct aw87319_container *aw87319_bspk_cnt;
...
static char *aw87319_bspk_name = "aw87319_bspk.bin";
...
static unsigned char aw87319_bspk_cfg_default[] = {
	0x9B, 0x07, 0x28, 0x05, 0x04, 0x0D, 0x03, 0x52, 0x28, 0x02
};
...
unsigned char aw87319_bspk_reg_val(unsigned char reg)
{
	if (aw87319->bspk_cfg_update_flag)
		return *(aw87319_bspk_cnt->data + reg);
	else
		return aw87319_bspk_cfg_default[reg];
}
...
unsigned char aw87319_audio_speaker(void)
{
	...
	aw87319->dynamic_boost_sence = SPEAKER_SCENE_PLAYBACK;
	return 0;
}

unsigned char aw87319_audio_dynamic_boost(int scene)
{
	...
	if (!aw87319->hwen_flag) {
		printk("aw87319_audio_dynamic_boost  IC do not power on\n");
		return 0;
	}

	if (aw87319->dynamic_boost_sence == scene) {
		return 0;
	}

	if (scene == SPEAKER_SCENE_BOOST) {
		/* Class D Enable; Boost Enable */
		i2c_write_reg(0x02, aw87319_bspk_reg_val(0x02));	/* BATSAFE */
		i2c_write_reg(0x03, aw87319_bspk_reg_val(0x03));	/* BOV */
		i2c_write_reg(0x04, aw87319_bspk_reg_val(0x04));	/* BP */
		i2c_write_reg(0x05, aw87319_bspk_reg_val(0x05));	/* Gain */
		i2c_write_reg(0x06, aw87319_bspk_reg_val(0x06));	/* AGC3_Po */
		i2c_write_reg(0x07, aw87319_bspk_reg_val(0x07));	/* AGC3 */
		i2c_write_reg(0x08, aw87319_bspk_reg_val(0x08));	/* AGC2 */
		i2c_write_reg(0x09, aw87319_bspk_reg_val(0x09));	/* AGC1 */
		/* CHIP Enable; Class D Enable; Boost Enable */
	} else {
		/* Class D Enable; Boost Enable */
		i2c_write_reg(0x02, aw87319_spk_reg_val(0x02));	/* BATSAFE */
	...
	aw87319->dynamic_boost_sence = scene;

	return 0;
}


unsigned char aw87319_audio_boost_speaker(void)
{
	if (aw87319 == NULL){
		return 2;
	}
	...
	i2c_write_reg(0x01, aw87319_bspk_reg_val(0x01) & 0xFB);
	/* Class D Enable; Boost Enable */
	i2c_write_reg(0x02, aw87319_bspk_reg_val(0x02));	/* BATSAFE */
	i2c_write_reg(0x03, aw87319_bspk_reg_val(0x03));	/* BOV */
	i2c_write_reg(0x04, aw87319_bspk_reg_val(0x04));	/* BP */
	...
	i2c_write_reg(0x08, aw87319_bspk_reg_val(0x08));	/* AGC2 */
	i2c_write_reg(0x09, aw87319_bspk_reg_val(0x09));	/* AGC1 */
	i2c_write_reg(0x01, aw87319_bspk_reg_val(0x01));
	/* CHIP Enable; Class D Enable; Boost Enable */

	aw87319->dynamic_boost_sence = SPEAKER_SCENE_BOOST;
					     ...
					     void *context)
{
 ...
 static void aw87319_boost_speaker_cfg_loaded(const struct firmware *cont,
					     void *context)
{
	unsigned int i;
	int ret;
	...
	aw87319_bspk_cnt = kzalloc(cont->size + sizeof(int), GFP_KERNEL);
	if (!aw87319_bspk_cnt) {
		release_firmware(cont);
		pr_err("%s: error allocating memory\n", __func__);
		return;
	}
	aw87319_bspk_cnt->len = cont->size;
	memcpy(aw87319_bspk_cnt->data, cont->data, cont->size);
	release_firmware(cont);

	for (i = 0; i < aw87319_bspk_cnt->len; i++) {
		pr_info("%s: vspk_cnt: addr:0x%02x, data:0x%02x\n", __func__, i,
			*(aw87319_bspk_cnt->data + i));
	}

	aw87319->bspk_cfg_update_flag = 1;
	ret =
					...
					void *context)
{
	...
	ret =
	    request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
				    aw87319_bspk_name,
				    &aw87319->i2c_client->dev, GFP_KERNEL, NULL,
				    aw87319_boost_speaker_cfg_loaded);
	if (ret) {
		aw87319->bspk_cfg_update_flag = 0;
		pr_err("%s: request_firmware_nowait failed with read %s",
		       __func__, aw87319_bspk_name);
	}
}
				...
				struct device_attribute *attr, char *buf)
{
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "4: vspk mode\n");
