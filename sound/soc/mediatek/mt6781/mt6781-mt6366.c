...
extern unsigned char aw87319_audio_boost_speaker(void);
extern unsigned char aw87319_audio_dynamic_boost(int scene);
...
static int aw87319_speaker_scene = SPEAKER_SCENE_PLAYBACK;
static const char *const aw87319_spk_scene[] = { "Playback", "Boost" };
static const struct soc_enum aw87319_spk_scene_enum =
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(aw87319_spk_scene), aw87319_spk_scene);

static int aw87319_spk_scene_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("%s() = %d\n", __func__, aw87319_speaker_scene);
	ucontrol->value.integer.value[0] = aw87319_speaker_scene;
	return 0;
}

static int aw87319_spk_scene_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("%s: ucontrol = %ld\n", __func__, ucontrol->value.integer.value[0]);

	if(SPEAKER_SCENE_NUM <= ucontrol->value.integer.value[0]) {
		aw87319_speaker_scene = SPEAKER_SCENE_PLAYBACK;
		pr_err("%s(): set speaker scene val = %ld !!! \r\n", __func__, ucontrol->value.integer.value[0]);
	} else {
		aw87319_speaker_scene = ucontrol->value.integer.value[0];
		aw87319_audio_dynamic_boost(aw87319_speaker_scene);
	}
				       ...
				       int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		/* spk amp on control */
		//#ifdef ODM_HQ_EDIT
		//#qiantao@ODM.HQ.Multimeida.Audio 2021/04/23 added for HQ audio bringup for aw87315
		if(aw87319_speaker_scene == SPEAKER_SCENE_PLAYBACK)
	...
	case SND_SOC_DAPM_PRE_PMD:
		/* spk amp off control */
		//#ifdef ODM_HQ_EDIT
		//#qiantao@ODM.HQ.Multimeida.Audio 2021/04/23 added for HQ audio bringup for aw/15
		aw87319_audio_off();
		//#endif /* ODM_HQ_EDIT */
		break;
	default:
...
static const struct snd_kcontrol_new mt6781_mt6366_controls[] = {
	...
	SOC_ENUM_EXT("AW87319 Spk Scene", aw87319_spk_scene_enum,
                   aw87319_spk_scene_get, aw87319_spk_scene_set),
};
