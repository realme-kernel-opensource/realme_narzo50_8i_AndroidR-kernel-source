...
#ifdef CONFIG_OPLUS_CHARGER_MTK6781
	if (pinfo->data.dual_charger_support) {//ljq
	...
	if (g_oplus_chip->ac_online != true) {
		...
		if (tcpc_dev) {
			if (!(tcpc_dev->pd_wait_hard_reset_complete)
					&& !/*oplus_mt6370_get_vbus_status()*/(bool (*) (void)) pmic_chrdet_status) {
				kernel_power_off();
				//mt_power_off();
			}
		}
...
/*zhaojianchuango@ODM.HQ.BSP.CHG 2021/05/08 modify link_temp*/
void oplus_link_temp_check(struct oplus_chg_chip *chip)
{
	int usbtemp_volt = 0;
	int i = 0;
	//int notify = 0; 
	...
	usbtemp_volt = oplus_link_ntc_adc2_read();//get flashlight volt
	if (usbtemp_volt <= 0) {
		usbtemp_volt = 367;		//25
	}
	chip->link_volt = usbtemp_volt;
	for (i = ARRAY_SIZE(con_volt_100K_spaceb) - 1; i >= 0; i--) {
		if (con_volt_100K_spaceb[i] >= chip->link_volt)
			break;
		else if (i == 0)
			break;
	}
	chip->link_temp = con_temp_100K[i];

	/*if(chip->link_temp < 80)
		notify = chip->notify_code;

	if(chip->link_temp >= 80)
		chip->notify_code |= 1 << NOTIFY_LINK_OVER_TEMP;
	else
		chip->notify_code = notify;*/
	printk(KERN_ERR "[OPLUS_CHG][%s]: link_temp = %d,link_volt =%d\n", __func__,chip->link_temp,chip->link_volt);
	return;
}
...
static void usbtemp_callback(unsigned long a)
{
	struct oplus_chg_chip *chip = g_oplus_chip;
		...
		if(chip->usb_temp_r < USB_57C && chip->usb_temp_l < USB_57C) {
			...
			chg_err("usbtemp exit fail usbtemp too high usb_temp[%d,%d], usb_volt[%d,%d] \n",
				...
				if (count_r >= RETRY_COUNT || count_l >= RETRY_COUNT) {
					if (!IS_ERR_OR_NULL(chip->normalchg_gpio.dischg_enable)) {
						...
						chg_err("dischg enable...usb_temp[%d,%d], usb_volt[%d,%d]\n",
								chip->usb_temp_r, chip->usb_temp_l, chip->usbtemp_volt_r, chip->usbtemp_volt_l);
	/*guohuiqing@ODM.HQ.BSP.CHG 2021/05/31 change for  high temp version*/
							 if (get_eng_version() != HIGH_TEMP_AGING) {
								chg_err("this is not CONFIG_HIGH_TEMP_VERSION\n");
				...
				count_l = 0;
				count = 0;
			} else if (((chip->usb_temp_r  - chip->temperature/10) > 12 && chip->usb_temp_r >= USB_30C && chip->usb_temp_r < USB_100C)
					|| ((chip->usb_temp_l  - chip->temperature/10) > 12 && chip->usb_temp_l >= USB_30C && chip->usb_temp_l < USB_100C)) {
				if (count <= time_count) {
					if (count == 0) {
	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_SUB_CURRENT:
		/*guohuiqing@ODM.HQ.BSP.CHG 2021/06/15	change for	get sub charge current */
		if (g_oplus_chip->charger_exist && bq25601d_is_detected() && (g_oplus_chip->slave_charger_enable || em_mode)) {
...
#endif
/*#if defined(VENDOR_EDIT) && defined(CONFIG_OPLUS_CHARGER_MTK6781)//zjc
...
#endif
#ifdef CONFIG_OPLUS_CHARGER_MTK6781
/*hongzhenglong@ODM.HQ.BSP.CHG 2021/07/20 add for capactity level*/
...
int oplus_chg_set_qc_config(void)
{
	...
	printk(KERN_ERR "%s:charger_volt:%d,temperature:%d,cool_down_force_5v:%d,batt_volt:%d",
...
void oplus_chg_set_camera_on(bool val)
{
	if (!g_oplus_chip) {
		...
		if (oplus_chg_get_voocphy_support() == true && is_spaceb_hc_project() == 1) {
			printk(KERN_ERR "%s: g_oplus_chip->camera_on = %d\n", __func__, g_oplus_chip->camera_on);
...
struct oplus_chg_operations  mtk6370_chg_ops = {
	...
	//.check_chrdet_status = oplus_mt6370_get_vbus_status,
	.check_chrdet_status = (bool (*) (void)) pmic_chrdet_status,//ljq
