...
extern struct pm_qos_request pm_qos_req;
...
extern int sc8547_init_vooc(void);

extern int mt6370_suspend_charger(bool suspend);
extern void  voocphy_cpufreq_update(int flag);
...
extern void oplus_reset_fastchg_after_usbout(void);//ljq
...
static bool is_allow_voocphy_fast_chg_real(void)
{
...
void oplus_vooc_switch_fast_chg(void)
{
	...
	if (oplus_chg_get_voocphy_support()) {
		...
		if (oplus_vooc_get_fastchg_to_normal()) {
			...
			if (is_allow_voocphy_fast_chg_real()) {
...
static int oplus_vooc_convert_fast_chg_type(int fast_chg_type)
{
	...
	if (!chip &&  (oplus_chg_get_voocphy_support() == false))
		return FASTCHG_CHARGER_TYPE_UNKOWN;
	...
	/*chg_err("oplus_vooc_convert_fast_chg_type:fast_chg_type:%d, fastchg_pwr_type:%d\n", fast_chg_type, fastchg_pwr_type);*/
