...
struct bat_gpio_pinctrl {
	int batid_gpio;
	int linktem_gpio;
	int linkbustem_gpio;
	struct pinctrl *pinctrl;
	struct pinctrl_state *batid_switch_enable;
	struct pinctrl_state *batid_switch_disable;
	struct pinctrl_state *linktem_switch_enable;
	struct pinctrl_state *linktem_switch_disable;
	struct pinctrl_state *linkbustem_switch_enable;
	struct pinctrl_state *linkbustem_switch_disable;
};
...
/* cust req ocv data */
	...
	/*hongzhenglong@ODM.HQ.BSP.CHG 2020/04/19 add for bringing up 18W&33W*/
	struct bat_gpio_pinctrl bat_gpio;
