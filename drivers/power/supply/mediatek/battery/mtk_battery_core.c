...
void fgauge_get_profile_id(void)
{
	...
	if (get_ec()->debug_bat_id_en == 1)
		...
		if (is_fuelgauge_apply() == true && is_spaceb_hc_project() == 1)
			battery_type_check();
		else
			gm.battery_id = 0;
#endif
...
void fg_custom_init_from_header(void)
{
	...
	if (is_spaceb_hc_project() == 1) {
		if (gm.battery_id == 3) {
			fg_cust_data.ui_full_limit_en = UI_FULL_LIMIT_EN3;
			fg_cust_data.ui_full_limit_soc0 = UI_FULL_LIMIT_SOC30;
			fg_cust_data.ui_full_limit_ith0 = UI_FULL_LIMIT_ITH30;
			fg_cust_data.ui_full_limit_soc1 = UI_FULL_LIMIT_SOC31;
			fg_cust_data.ui_full_limit_ith1 = UI_FULL_LIMIT_ITH31;
			fg_cust_data.ui_full_limit_soc2 = UI_FULL_LIMIT_SOC32;
			fg_cust_data.ui_full_limit_ith2 = UI_FULL_LIMIT_ITH32;
			fg_cust_data.ui_full_limit_soc3 = UI_FULL_LIMIT_SOC33;
			fg_cust_data.ui_full_limit_ith3 = UI_FULL_LIMIT_ITH33;
			fg_cust_data.ui_full_limit_soc4 = UI_FULL_LIMIT_SOC34;
			fg_cust_data.ui_full_limit_ith4 = UI_FULL_LIMIT_ITH34;
		}
	}
	...
	/*hongzhenglong@ODM.HQ.BSP.CHG 2020/04/19 add for bringing up 18W&33W*/
	if(is_spaceb_hc_project() == 1 && get_PCB_Version() < 4){
...
void fg_custom_init_from_dts(struct platform_device *dev)
{
	...
	if (is_spaceb_hc_project() == 1) {
		sprintf(node_name, "DIFFERENCE_FULLOCV_ITH%d", bat_id);
	...
	if (is_spaceb_hc_project() == 1) {
		sprintf(node_name, "EMBEDDED_SEL%d", bat_id);
		fg_read_dts_val(np, node_name, &(fg_cust_data.embedded_sel), 1);
	}
