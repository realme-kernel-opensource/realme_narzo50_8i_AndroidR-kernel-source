...
unsigned int TempToBattVolt(int temp, int update)
{
	...
	/*bm_err("[RBAT_PULL_UP_VOLT_BY_BIF]vbif28 = %d /n",vbif28);*/
#endif
...
int force_get_tbat_internal(bool update)
{
	...
	if (update == true || pre_bat_temperature_val == -1) {
		...
		if (bat_temperature_volt != 0) {
			...
			bm_err("[force_get_tbat_internal] tbat_precise bat_temperature_val:%d, bat_temperature_volt:%d\n",
				bat_temperature_val, bat_temperature_volt);
#else
	...
	struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case Get_FakeOff_Param:
		...
		if (mt6370_get_vbus_status() == true && oplus_get_prop_status() != POWER_SUPPLY_STATUS_NOT_CHARGING) {
...
static int meter_fg_30_get_prev_batt_fcc(void)
{
	return BATT_CAPACITY;
}
...
{
	printk("last full = %d, full = %d, enable_is_force_full = %d\n", last_full, full, enable_is_force_full);
	if(last_full != full) {
		if (full) {
...
/*guohuiqing@ODM.HQ.BSP.CHG 2021/05/08 add for batid*/

static int oplus_batid_gpio_enable(void)
{
	printk(KERN_ERR "[OPLUS_CHG][%s]: oplus_bat_id_tem_gpio_init!\n", __func__);

if (IS_ERR_OR_NULL(gm.bat_gpio.pinctrl)) {
		printk("oplus_batid_gpio_enable get gm.bat_gpio.pinctrl   fail\n");
		return -EINVAL;
	}

if(gm.bat_gpio.batid_gpio)
	pinctrl_select_state(gm.bat_gpio.pinctrl,
		gm.bat_gpio.batid_switch_enable);

if(gm.bat_gpio.linktem_gpio)
	pinctrl_select_state(gm.bat_gpio.pinctrl,
		gm.bat_gpio.linktem_switch_disable);

if(gm.bat_gpio.linkbustem_gpio)
     pinctrl_select_state(gm.bat_gpio.pinctrl,
		gm.bat_gpio.linkbustem_switch_disable);
...
int oplus_linktem_gpio_enable(void)
{
	printk(KERN_ERR "[OPLUS_CHG][%s]: oplus_bat_id_tem_gpio_init!\n", __func__);
	...
	if(gm.bat_gpio.batid_gpio)
	pinctrl_select_state(gm.bat_gpio.pinctrl,
		gm.bat_gpio.batid_switch_disable);

	if(gm.bat_gpio.linktem_gpio)
		pinctrl_select_state(gm.bat_gpio.pinctrl,
			gm.bat_gpio.linktem_switch_enable);
...
/*static int oplus_linkbustem_gpio_enable(void)
{
	printk(KERN_ERR "[OPLUS_CHG][%s]: oplus_bat_id_tem_gpio_init!\n", __func__);
	if (IS_ERR_OR_NULL(gm.bat_gpio.pinctrl)) {
			printk("oplus_batid_gpio_enable get gm.bat_gpio.pinctrl   fail\n");
	...
	if(gm.bat_gpio.linkbustem_gpio)
	pinctrl_select_state(gm.bat_gpio.pinctrl,
		gm.bat_gpio.linkbustem_switch_enable);
...
int oplus_bat_id_tem_gpio_disable(void)
{
	bm_err( "[OPLUS_CHG][%s]: oplus_bat_id_tem_gpio_disable!\n", __func__);
...
static int oplus_bat_id_tem_gpio_init(struct device *dev)
{
	printk( "[OPLUS_CHG][%s]: oplus_bat_id_tem_gpio_init!:dev[0x%x ]\n", __func__,dev);
       gm.bat_gpio.pinctrl = devm_pinctrl_get(dev);

	if (IS_ERR_OR_NULL(gm.bat_gpio.pinctrl)) {
		printk("get gm.bat_gpio.pinctrl   fail\n");
		return -EINVAL;
	}

//BAT_ID
	gm.bat_gpio.batid_switch_disable=
		pinctrl_lookup_state(	gm.bat_gpio.pinctrl ,
			"batid_disable");

	if (IS_ERR_OR_NULL(gm.bat_gpio.batid_switch_disable)) {
		bm_err("get batid_disable fail\n");
		return -EINVAL;
	}
	...
	if (IS_ERR_OR_NULL(gm.bat_gpio.batid_switch_enable)) {
		bm_err("get batid_enable fail\n");
		return -EINVAL;
	}
...
//LINKTEM
	...
	if (IS_ERR_OR_NULL(gm.bat_gpio.linktem_switch_enable)) {
		bm_err("get linktem_enable fail\n");
		return -EINVAL;
	}
	...
	if (IS_ERR_OR_NULL(gm.bat_gpio.linktem_switch_disable)) {
		bm_err("get linktem_disable fail\n");
		return -EINVAL;
	}
...
//LINKBUSTEM
	...
	if (IS_ERR_OR_NULL(gm.bat_gpio.linkbustem_switch_enable)) {
		bm_err("get linkbustem_enable fail\n");
		return -EINVAL;
	}
	...
	if (IS_ERR_OR_NULL(gm.bat_gpio.linkbustem_switch_disable)) {
		bm_err("get linkbustem_disable fail\n");
		return -EINVAL;
	}

	 oplus_bat_id_tem_gpio_disable();

	 printk( "[OPLUS_CHG][%s]: sucess\n", __func__);
	 return 0;
}


static int oplus_bat_id_tem_parse_dt(struct device *dev)
{
	int rc = 0;
	struct device_node *node = NULL;

       printk( "[OPLUS_CHG][%s]:dev[0x%x	] \n", __func__,dev);

	node = dev->of_node;
	if (node == NULL) {
		printk( "[OPLUS_CHG][%s]:	node is missing 0x%x  \n", __func__,node);
		return -EINVAL;
	}

	printk( "[OPLUS_CHG][%s]:	0x%x  \n", __func__,node);
//batid
	gm.bat_gpio.batid_gpio=
			of_get_named_gpio(node, "qcom,batid-gpio", 0);
	if (gm.bat_gpio.batid_gpio <= 0) {
			bm_err("Couldn't read qcom,batid-gpio rc = %d, qcom,batid-gpio:%d\n",
					rc, gm.bat_gpio.batid_gpio);
			return -EINVAL;
	}
//linktem
	gm.bat_gpio.linktem_gpio=
			of_get_named_gpio(node, "qcom,linktem-gpio", 0);

	if (gm.bat_gpio.linktem_gpio <= 0) {
			bm_err("Couldn't read qcom,linktem-gpio rc = %d, qcom,linktem-gpio:%d\n",
					rc, gm.bat_gpio.linktem_gpio);
			return -EINVAL;
	}

//linkbuste
	gm.bat_gpio.linkbustem_gpio=
				of_get_named_gpio(node, "qcom,linkbustem-gpio", 0);
	if (gm.bat_gpio.linkbustem_gpio <= 0) {
			bm_err("Couldn't read qcom,linkbustem-gpio rc = %d, qcom,linkbustem-gpio:%d\n",
					rc, gm.bat_gpio.linkbustem_gpio);
			return -EINVAL;
	}

	printk("oplus_bat_id_tem_parse_dt  gpio :batid[%d],linktem[%d],linkbustem[%d] \n",gm.bat_gpio.batid_gpio,gm.bat_gpio.linktem_gpio,gm.bat_gpio.linkbustem_gpio);

	if (gpio_is_valid(gm.bat_gpio.batid_gpio)&&gpio_is_valid(gm.bat_gpio.linkbustem_gpio)  && gpio_is_valid(gm.bat_gpio.linktem_gpio)  )
       {
                     rc = gpio_request(gm.bat_gpio.batid_gpio, "batid-gpio");
			if (rc) {
				bm_err("unable to request batid-gpio:%d\n",gm.bat_gpio.batid_gpio);
				return -EINVAL;
			}

                     rc = gpio_request(gm.bat_gpio.linktem_gpio, "linktem-gpio");
			if (rc) {
				bm_err("unable to request linktem-gpio:%d\n",gm.bat_gpio.linktem_gpio);
				return -EINVAL;
			}

                     rc = gpio_request(gm.bat_gpio.linkbustem_gpio, "linkbustem-gpio");
			if (rc) {
				bm_err("unable to request linkbustem-gpio:%d\n",gm.bat_gpio.linkbustem_gpio);
				return -EINVAL;
			}

	}

      bm_err("oplus_bat_id_tem_parse_dt success end\n");
      return 0;
}
...
int battery_type_check(void)
{
	...
	#else
	 if (batt_id == NULL) {
		bm_debug("[battery_type_check]: batt_id adc channel is null\n");
		gm.battery_id = 0;
		return BAT_TYPE__ATL_4400mV;
	}

	if(is_spaceb_hc_project() == 1){/*33w*/
		oplus_batid_gpio_enable();
	}
	...
	if(is_spaceb_hc_project() == 1){/*33w*/
		oplus_bat_id_tem_gpio_disable();
		oplus_linktem_gpio_enable();
	}

	if (ret < 0) {
		bm_debug( "[battery_type_check] read channel err = %d,\n", ret);
	}
	printk( "[battery_type_check]: ret = %d,ret_value[%d]\n", ret,ret_value);
	value = ret_value * 1500 / 4096;
	#endif
	printk("[battery_value= %d\n", value);
	if(is_fuelgauge_apply() == true){
		...
		/*hongzhenglong@ODM.HQ.BSP.CHG 2020/04/19 add for bringing up 18W&33W*/
		if(is_spaceb_hc_project() == 1){//33w
			if (value >= BATID_VOLT_ALT_MIN_33W && value <= BATID_VOLT_ALT_MAX_33W) {//ATL  68K  0.55-0.79V  0.68
					battery_type = BAT_TYPE__ATL_4450mV;
					gm.battery_id = 3;
			 }else {
...
//#ifdef CONFIG_OPLUS_CHARGER_MTK6781
	/*bringup space-b add Temporary battery*/
	//if (battery_type == BAT_TYPE__UNKNOWN) {
//#else
	if (battery_type == BAT_TYPE__ATL_4450mV || battery_type == BAT_TYPE__COS_4450mV || battery_type == BAT_TYPE__LIW_4450mV) {
//#endif
		return true;
	} else {
		if (retry_flag == 0) {
...
static struct oplus_gauge_operations battery_meter_fg_30_gauge = {
	...
	.get_prev_batt_fcc 				= meter_fg_30_get_prev_batt_fcc,
...
/*Baoquan.Lai@BSP.CHG.Basic 2020/12/25 add for force full*/
	...
	if((!oplus_bat_id_tem_parse_dt(&dev->dev)) && (is_spaceb_hc_project() == 1)){
		         oplus_bat_id_tem_gpio_init(&dev->dev);
       }


	batt_id = iio_channel_get(&dev->dev, "auxadc2-batt_id_v");
	if (IS_ERR(batt_id)){
	...
	if (batt_id) {
		...
		bm_err("battery ID register_battery_devinfo success:%d\n",batt_id);
	}
