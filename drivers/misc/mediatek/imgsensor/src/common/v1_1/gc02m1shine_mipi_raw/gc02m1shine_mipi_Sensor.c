...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC02M1SHINE_SENSOR_ID,
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			printk("gc02m1shine module_id: 0x%x\n", imgsensor_info.module_id);
			*sensor_id = (return_sensor_id() + 2);
			if ((*sensor_id == imgsensor_info.sensor_id) && (imgsensor_info.module_id == MODULE_ID_SHINE)) {
				...
				printk("gc02m1shine i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
...
UINT32 GC02M1SHINE_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
