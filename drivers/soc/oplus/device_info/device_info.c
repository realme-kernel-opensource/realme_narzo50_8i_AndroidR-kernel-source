...
struct sensor_devinfo sensordevinfo[] = {
	...
	{"bmi2xy_acc","BOSCH"},
	{"bmi2xy_gyro","BOSCH"},
	{"icm4n607_acc","invensense"},
	{"icm4n607_gyro","invensense"},
	{"lsm6ds3","ST"},
};
...
{
	int i = 0, isupport = 0 , ret = 0;
	int gpio, id = 0;
	int count = 0;
	char tmp[INFO_LEN] = { 0 };

	if (is_project(21684) || is_project(21685) || is_project(21686) || is_project(21687) ||
	    is_project(21690) || is_project(21691) || is_project(21692)) {
		isupport = 1;
	} else {
		isupport = BOARD_GPIO_SUPPORT;
	}

	for (i = 0; i < isupport; i++) {
...
reinit_aboard_id(struct device *dev, struct manufacture_info *info)
{
	...
	if ((is_project(21684) || is_project(21685) || is_project(21686) || is_project(21687) ||
		 is_project(21690) || is_project(21691) || is_project(21692)) && (get_PCB_Version() >= PCB_VERSION_EVT2)) {
		id_size = of_property_count_elems_of_size(np, "aboard-patterns-evt2", sizeof(uint32_t));
	} else {
		id_size = of_property_count_elems_of_size(np, "aboard-patterns", sizeof(uint32_t));
	}
	/* #endif OPLUS_DEVICE_IFNO */
	...
	if ((is_project(21684) || is_project(21685) || is_project(21686) || is_project(21687) ||
		 is_project(21690) || is_project(21691) || is_project(21692)) && (get_PCB_Version() >= PCB_VERSION_EVT2)) {
		of_property_read_u32_array(np, "aboard-patterns-evt2", sub_val, id_size);
	} else {
	...
	dev_msg("devinfo: reinit_aboard_id sub_val id_size:%d main_val id_size:%d\n", id_size, id_size);
...
/*#endif OPLUS_FEATURE_TP_BASIC*/
	...
	dev_msg("aboard mask 0x%x  rf_val=%d *sub_val =%d\n", hw_mask,rf_val,*sub_val);
	dev_msg("get_project() value is = %d\n", get_project());
	dev_msg("get_Modem_Version() value is = %d\n", get_Modem_Version());
...
{
	dev_msg("device_info_init\n");
	g_parent = proc_mkdir("devinfo", NULL);

	if (!g_parent) {
		return -ENODEV;
	}
	proc_create("devinfo/Sensor_accel", 0644,  NULL, proc_fops_list[0]);
	proc_create("devinfo/Sensor_alsps", 0644,  NULL, proc_fops_list[1]);
	proc_create("devinfo/Sensor_gyro", 0644,  NULL, proc_fops_list[2]);
	proc_create("devinfo/Sensor_msensor", 0644,  NULL, proc_fops_list[3]);
