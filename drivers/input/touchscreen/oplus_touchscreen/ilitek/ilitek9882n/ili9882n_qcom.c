...
/* jinjiahuan@BSP.TP.Driver 2021/07/01 add for space gesture flag after FW upgrade */
    ILI_INFO("gesture_enable = %d\n",ilits->ts->gesture_enable);
	if (ilits->ts->gesture_enable > 0){
        ilits->ts->gesture_enable = 1;
        ILI_INFO("set gesture_enable = %d\n",ilits->ts->gesture_enable);
	}
