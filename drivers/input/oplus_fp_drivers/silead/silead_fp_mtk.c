...
static int silfp_parse_dts(struct silfp_data* fp_dev)
{
	...
	if( SIL_LDO_DISBALE == 0 ) {
	   /* fp_dev->pin.spi_default = pinctrl_lookup_state(fp_dev->pin.pinctrl, "default");
	    if (IS_ERR(fp_dev->pin.spi_default)) {
	        ret = PTR_ERR(fp_dev->pin.spi_default);
	        pr_info("%s can't find silfp spi-default\n", __func__);
	        return ret;
	    }*/
	    ret = of_property_read_u32(node, "sil,vmch_enable", &vmch_enable);
		if (ret) {
	...
	if ( SIL_LDO_DISBALE == 0 ) {
        ...
        case 1:
            ...
            break;
        case 2:
            fp_dev->avdd_ldo = regulator_get_optional(NULL, "irtx_ldo");
            break;
        default:
