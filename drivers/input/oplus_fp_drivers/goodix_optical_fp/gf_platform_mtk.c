    ...
    if (enable) {
        if (!vreg) {
            ...
            if (IS_ERR(vreg)) {
                ...
                vreg = regulator_get_optional(NULL, "irtx_ldo");
                if (IS_ERR(vreg)) {
                    pr_err("Unable to get  %s\n", name);
                    return PTR_ERR(vreg);
                }
...
int gf_parse_dts(struct gf_dev* gf_dev)
{
    ...
    if (FP_GOODIX_5658 == get_fpsensor_type() || FP_GOODIX_3626 == get_fpsensor_type() || FP_GOODIX_3956 == get_fpsensor_type()) { //add new fpsensor if needed
