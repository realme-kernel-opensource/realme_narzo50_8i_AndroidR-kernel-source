...
// #endif
// #ifdef OPLUS_BUG_COMPATIBILITY
// wangcheng@MULTIMEDIA.DISPLAY.LCD 2021/08/12 add solve low-brightness splash screen and suspend backlight is bright
...
// #endif
// wangcheng@MULTIMEDIA.DISPLAY.LCD 2021/06/29 add for keep cabc mode after resume
// #ifdef OPLUS_BUG_COMPATIBILITY
...
// wangcheng@MULTIMEDIA.DISPLAY.LCD 2021/07/27 backlight curve remapping in low brightness range
// #ifdef OPLUS_BUG_COMPATIBILITY
	27,2,
	14,8,
	16,8,
...
//#ifdef OPLUS_BUG_COMPATIBILITY
// wangcheng@MULTIMEDIA.DISPLAY.LCD 2021/06/4 modified for space resume  quick
...
//#ifdef OPLUS_BUG_COMPATIBILITY
// wangcheng@MULTIMEDIA.DISPLAY.LCD 2021/08/1 modified for space backlight control by i2c_pmic
	lcm_dcs_write_seq_static(ctx,0xFF,0x78,0x07,0x06); // pwm freq 28k
	lcm_dcs_write_seq_static(ctx,0x08,0x20);  //pwm osc 116
//#endif
//#ifdef OPLUS_BUG_COMPATIBILITY
// wangcheng@MULTIMEDIA.DISPLAY.LCD 2021/07/13 add for space  check more esd register
...
//#ifdef OPLUS_BUG_COMPATIBILITY
// wangcheng@MULTIMEDIA.DISPLAY.LCD 2021/06/2 modified for space pwm duty rise timing
    lcm_dcs_write_seq_static(ctx,0x84,0x01);  //28K
// #endif
// wangcheng@MULTIMEDIA.DISPLAY.LCD 2021/07/13 modified for space cabc v3 para
//#ifdef OPLUS_BUG_COMPATIBILITY
    lcm_dcs_write_seq_static(ctx,0xFF,0x78,0x07,0x03);
...
// wangcheng@MULTIMEDIA.DISPLAY.LCD 2021/06/9 add for space  Sleep in refresh 2frame blank
//#ifdef OPLUS_BUG_COMPATIBILITY
...
// wangcheng@MULTIMEDIA.DISPLAY.LCD 2021/06/7 add for space esd
//#ifdef OPLUS_BUG_COMPATIBILITY
...
static int lcm_unprepare(struct drm_panel *panel)
{
	...
	if (!ctx->prepared)
		return 0;
//#ifdef OPLUS_BUG_COMPATIBILITY
// wangcheng@MULTIMEDIA.DISPLAY.LCD 2021/06/7 modified for space avoid sleep cmd fail
	udelay(1 * 1000);
// #endif
//ifdef OPLUS_BUG_COMPATIBILITY
...
// lihao@MULTIMEDIA.DISPLAY.LCD 2021/07/30 Turn on black screen gesture shutdown timing modification and esd recorvery brightness
// #ifdef OPLUS_BUG_COMPATIBILITY
	if ((0 == tp_gesture_enable_flag()) || (1 == shutdown_flag)  || (esd_flag == 1)) {
// #endif
//#ifdef OPLUS_BUG_COMPATIBILITY
//wangcheng@MULTIMEDIA.DISPLAY.LCD 2021/07/22 Modify rst after mipi is powered off
	ctx->reset_gpio =
		devm_gpiod_get(ctx->dev, "reset", GPIOD_OUT_HIGH);
	if (IS_ERR(ctx->reset_gpio)) {
		dev_err(ctx->dev, "%s: cannot get reset_gpio %ld\n",
...
// liuxing@MULTIMEDIA.DISPLAY.LCD 2021/08/06 add for keep cabc mode after resume
// #ifdef OPLUS_BUG_COMPATIBILITY
...
// #endif
	...
	lcm_panel_init(ctx);
// #ifdef OPLUS_BUG_COMPATIBILITY
// wangcheng@MULTIMEDIA.DISPLAY.LCD 2021/08/12 add resolve suspend backlight is bright
...
// wangcheng@MULTIMEDIA.DISPLAY.LCD 2021/07/21 modified for ensure that the mipi line time is consistent with the IC setting
// #ifdef OPLUS_BUG_COMPATIBILITY
#define HFP (86)
#define HSA (4)
#define HBP (86)
...
//#endif
// #ifdef OPLUS_BUG_COMPATIBILITY
// wangcheng@MULTIMEDIA.DISPLAY.LCD 2021/08/06 add solve low-brightness splash screen
	if (level < 265) {
...
// #endif
//#ifdef OPLUS_BUG_COMPATIBILITY
// wangcheng@MULTIMEDIA.DISPLAY.LCD 2021/06/2 modified for space min brightness 1nits
	if (level > 2047){
		level = 2047;
// wangcheng@MULTIMEDIA.DISPLAY.LCD 2021/07/27 backlight curve remapping in low brightness range
// #ifdef OPLUS_BUG_COMPATIBILITY
...
// wangcheng@MULTIMEDIA.DISPLAY.LCD 2021/07/23 add it takes 50ms for the delay from mipi high data to backlight
// #ifdef OPLUS_BUG_COMPATIBILITY
...
// wangcheng@MULTIMEDIA.DISPLAY.LCD 2021/07/21 modified pixel clock = htotal * vtotal * vrefresh / 1000
// #ifdef OPLUS_BUG_COMPATIBILITY
	.clock = 375444,
// #endif
	.hdisplay = HAC,
...
static struct mtk_panel_params ext_params_120hz = {
	.pll_clk = 553,
// wangcheng@MULTIMEDIA.DISPLAY.LCD 2021/06/22 add for space  mipi timing
// #ifdef OPLUS_BUG_COMPATIBILITY
	.phy_timcon = {
	    .hs_trail = 20,
	},
...
// #endif
	...
	.data_rate = 1106,
	.dyn_fps = {
// wangcheng@MULTIMEDIA.DISPLAY.LCD 2021/07/21 modified clk working at 120hz, you only need to switch vfp to cut the frame rate.
// #ifdef OPLUS_BUG_COMPATIBILITY
		.switch_en = 1, .vact_timing_fps = 120,
// #endif
	},
...
// wangcheng@MULTIMEDIA.DISPLAY.LCD 2021/07/07 modified for mipi hopping frame rate mismatch
// #ifdef OPLUS_BUG_COMPATIBILITY
