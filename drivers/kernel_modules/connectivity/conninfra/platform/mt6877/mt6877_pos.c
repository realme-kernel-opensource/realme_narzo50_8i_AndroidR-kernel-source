...
void consys_set_if_pinmux_mt6877(unsigned int enable)
{
	...
	if (enable) {
		 ...
		 * conn_hrst_b(PAD_CONN_HRST_B)
		 * conn_top_clk(PAD_CONN_TOP_CLK)
		 * 0x11EA_0004=32'h08000000 ([29:27] is for conn)
		 */
		CONSYS_REG_WRITE(IOCFG_RT_DRV_CFG0_SET, 0x08000000);
		...
		/* if(TCXO mode)
		 * 	set GPIO189 pinmux for TCXO mode (Aux2)(CONN_TCXOENA_REQ)
		 */
		...
		if (consys_co_clock_type_mt6877() == CONNSYS_CLOCK_SCHEMATIC_26M_EXTCXO) {
			...
			/* 0x1000_5470[22:20]=3'b010 */
			CONSYS_REG_WRITE_MASK(GPIO_MODE23, 0x200000, 0x700000);
	#else
		...
		/* For CTP load, write CR directly */
			/* if(TCXO mode)
			 * 	Set GPIO189 pinmux for TCXO mode (Aux0)(CONN_TCXOENA_REQ)
			 * 0x1000_5470[22:20]=3'b000
			 */
			CONSYS_REG_WRITE_MASK(GPIO_MODE23, 0, 0x700000);
	#else
			if (!IS_ERR(g_conninfra_pinctrl_ptr)) {
...
int connsys_a_die_cfg_mt6877(void)
{
	...
	if (adie_26m) {
		...
		CONSYS_REG_WRITE_MASK(GPIO_MODE25, 0x0, 0x10000);
	}
...
int connsys_low_power_setting_mt6877(unsigned int curr_status, unsigned int next_status)
{
	...
	if (curr_status == 0) {
		 ...
		 * 		0x18000020[7] = 1'b0 (with key wdata[31:15] = 0x5746)
		 * 	CONN_INFRA_RGU_WFFYS_ON_TOP_PWR_CTL_BGFSYS_ON_TOP_PWR_ACK_S_MASK
		 * 		0x18000010[7] = 1'b0 (with key wdata[31:15] = 0x4254)
		 */
		CONSYS_REG_WRITE_MASK(CONN_RGU_BGFYS_ON_TOP_PWR_CTL_ADDR, 0x57460000, 0xffff0080);
		CONSYS_REG_WRITE_MASK(CONN_RGU_WFSYS_ON_TOP_PWR_CTL_ADDR, 0x42540000, 0xffff0080);
		 ...
		 * CONN_INFRA_OFF_TIMEOUT_LIMIT(0x1800_E300[14:7]) = 8'h31
		 */
		 ...
		 * CONN_INFRA_OFF_TIMEOUT_WAY_EN
		 * 	0x1800_E300[3:0] = 4'hf
		 */
		CONSYS_REG_WRITE_MASK(CONN_BUS_CR_CONN_INFRA_BUS_OFF_TIMEOUT_CTRL_ADDR, 0xf, 0xf);

		/* Set conn_infra_on bus timeout
		 * CONN_INFRA_ON_TIMEOUT_LIMIT(0x1800_E31C[14:7]) = 8'hc
		 */
		CONSYS_REG_WRITE_HW_ENTRY(
		 ...
		 * CONN_INFRA_ON_TIMEOUT_WAY_EN
		 * 	0x1800_E31C[3:0] = 4'hf
		 */
		CONSYS_REG_WRITE_MASK(
			CONN_BUS_CR_CONN_INFRA_BUS_ON_TIMEOUT_CTRL_ADDR, 0xf, 0xf);
		 ...
		 * CONN_INFRA_CLKGEN_ON_TOP_CKGEN_RFSPI_WPLL_DIV_WPLL_DIV_SEL(0x1800_9040[7:2]) = 6'h8
		 */
		CONSYS_REG_WRITE_HW_ENTRY(CONN_CLKGEN_ON_TOP_CKGEN_RFSPI_WPLL_DIV_WPLL_DIV_SEL, 0x8);

		/* Enable rfspis wpll div
		 * CONN_INFRA_CLKGEN_ON_TOP_CKGEN_RFSPI_WPLL_DIV_DIV_EN(0x1800_9040[0]) = 1'b1
		 */
		CONSYS_SET_BIT(CONN_CLKGEN_ON_TOP_CKGEN_RFSPI_WPLL_DIV_ADDR, (0x1 << 0));
