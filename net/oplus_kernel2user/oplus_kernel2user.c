...
void send_score(int link_index, int score)
{
    ...
    notify_user_retransmit.para_one = oplus_foreground_uid; // uid
    notify_user_retransmit.para_two = score; //score
    oplus_kernel_send_to_user(OPLUS_SEND_TCP_RETRANSMIT, (char *)&notify_user_retransmit, sizeof(notify_user_retransmit));
}
...
int oplus_kernel_send_to_user(int msg_type, char *payload, int payload_len)
{
    ...
    if(oplus_foreground_uid){
        ret = netlink_unicast(oplus_kernel2user_sock, skbuff, special_pid, MSG_DONTWAIT);
    } else {
        printk(KERN_ERR "oplus_kernel2user_netlink: can not unicast skbuff, oplus_foreground_uid=0\n");
    }
    if(ret < 0){
...
{
    oplus_kernel2user_table_hrd = register_net_sysctl(&init_net, "net/oplus_kernel2user",
...
static int oplus_kernel2user_netlink_init(void)
{
    ...
    oplus_kernel2user_sock = netlink_kernel_create(&init_net, NETLINK_OPLUS_KERNEL2USER, &cfg);
