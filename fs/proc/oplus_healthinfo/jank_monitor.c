...
void update_jank_trace_info(struct task_struct *tsk, int trace_type, unsigned int cpu, u64 delta)
{
	...
	if (trace_type == JANK_TRACE_RUNNABLE) { // runnable
		tsk->oplus_jank_info.runnable_state           += delta;
	} else if (trace_type == JANK_TRACE_DSTATE) { // D state
		tsk->oplus_jank_info.d_state.cnt++;
		if (tsk->in_iowait) {
			tsk->oplus_jank_info.d_state.iowait_ns    += delta;
		} else if (tsk->in_mutex) {
			tsk->oplus_jank_info.d_state.mutex_ns     += delta;
		} else if (tsk->in_downread) {
			tsk->oplus_jank_info.d_state.downread_ns  += delta;
		} else if (tsk->in_downwrite) {
			tsk->oplus_jank_info.d_state.downwrite_ns += delta;
		} else {
			tsk->oplus_jank_info.d_state.other_ns     += delta;
		}
	} else if (trace_type == JANK_TRACE_SSTATE) { // S state
		tsk->oplus_jank_info.s_state.cnt++;
		if (tsk->in_binder) {
			tsk->oplus_jank_info.s_state.binder_ns    += delta;
		} else if (tsk->in_futex) {
			tsk->oplus_jank_info.s_state.futex_ns     += delta;
		} else if (tsk->in_epoll) {
			tsk->oplus_jank_info.s_state.epoll_ns     += delta;
		} else {
			tsk->oplus_jank_info.s_state.other_ns     += delta;
		}
...
#endif /*CONFIG_OPLUS_SYSTEM_KERNEL_QCOM*/
		if (ltt_cpu) {
			tsk->oplus_jank_info.ltt_running_state += delta;
		} else {
			tsk->oplus_jank_info.big_running_state += delta;
		}
	}
}
...
static int proc_jank_trace_show(struct seq_file *m, void *v)
{
	...
	task_lock(p);
	iow_time = p->oplus_jank_info.d_state.iowait_ns;
	binder_time = p->oplus_jank_info.s_state.binder_ns;
	futex_time = p->oplus_jank_info.s_state.futex_ns;

	d_time = p->oplus_jank_info.d_state.iowait_ns + p->oplus_jank_info.d_state.mutex_ns +
		p->oplus_jank_info.d_state.downread_ns + p->oplus_jank_info.d_state.downwrite_ns +
		p->oplus_jank_info.d_state.other_ns;

	s_time = p->oplus_jank_info.s_state.binder_ns + p->oplus_jank_info.s_state.futex_ns +
		p->oplus_jank_info.s_state.epoll_ns + p->oplus_jank_info.s_state.other_ns;

	ltt_time = p->oplus_jank_info.ltt_running_state;

	big_time = p->oplus_jank_info.big_running_state;

	rn_time = p->oplus_jank_info.runnable_state;
				...
				size_t count, loff_t *ppos)
{
	...
	if (jank_trace == 1) {
		...
		task->jank_trace = 0;
		memset(&task->oplus_jank_info, 0, sizeof(struct oplus_jank_monitor_info));
	}
